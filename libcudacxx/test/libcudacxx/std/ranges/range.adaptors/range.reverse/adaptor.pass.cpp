...
struct Pred {
  __host__ __device__ int operator()(int i) const noexcept { return i; }
};
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  ...
  // sized subrange over reverse iterators
  {
    ...
    {
      BidirRange view(buf, buf + 3);
      ReverseSubrange subrange(ReverseIt(cuda::std::ranges::end(view)), ReverseIt(cuda::std::ranges::begin(view)), /* size */3);
    ...
    }
    {
      ...
      BidirRange view(buf, buf + 3);
      ReverseSubrange const subrange(ReverseIt(cuda::std::ranges::end(view)), ReverseIt(cuda::std::ranges::begin(view)), /* size */3);
      decltype(auto) result = cuda::std::views::reverse(subrange);
      static_assert(cuda::std::same_as<decltype(result), Subrange>);
      assert(base(result.begin()) == buf);
      assert(base(result.end()) == buf + 3);
  ...
  // unsized subrange over reverse iterators
  {
    ...
    using ReverseIt = cuda::std::reverse_iterator<It>;
    using ReverseSubrange = cuda::std::ranges::subrange<ReverseIt, ReverseIt, cuda::std::ranges::subrange_kind::unsized>;
