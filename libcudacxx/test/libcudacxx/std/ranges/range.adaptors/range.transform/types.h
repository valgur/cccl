...
struct MoveOnlyView : cuda::std::ranges::view_base {
  int start_;
  int *ptr_;
  __host__ __device__ constexpr explicit MoveOnlyView(int* ptr = globalBuff, int start = 0) : start_(start), ptr_(ptr) {}
  constexpr MoveOnlyView(MoveOnlyView&&) = default;
  constexpr MoveOnlyView& operator=(MoveOnlyView&&) = default;
  __host__ __device__ constexpr int *begin() const { return ptr_ + start_; }
  __host__ __device__ constexpr int *end() const { return ptr_ + 8; }
};
...
struct ForwardView : cuda::std::ranges::view_base {
  int *ptr_;
  __host__ __device__ constexpr explicit ForwardView(int* ptr = globalBuff) : ptr_(ptr) {}
  constexpr ForwardView(ForwardView&&) = default;
  constexpr ForwardView& operator=(ForwardView&&) = default;
  __host__ __device__ constexpr auto begin() const { return forward_iterator<int*>(ptr_); }
  __host__ __device__ constexpr auto end() const { return forward_iterator<int*>(ptr_ + 8); }
};
...
struct InputView : cuda::std::ranges::view_base {
  int *ptr_;
  __host__ __device__ constexpr explicit InputView(int* ptr = globalBuff) : ptr_(ptr) {}
  __host__ __device__ constexpr auto begin() const { return cpp20_input_iterator<int*>(ptr_); }
  __host__ __device__ constexpr auto end() const { return sentinel_wrapper<cpp20_input_iterator<int*>>(cpp20_input_iterator<int*>(ptr_ + 8)); }
};
...
struct SizedSentinelView : cuda::std::ranges::view_base {
  int count_;
  __host__ __device__ constexpr explicit SizedSentinelView(int count = 8) : count_(count) {}
  __host__ __device__ constexpr auto begin() const { return RandomAccessIter(globalBuff); }
  __host__ __device__ constexpr int *end() const { return globalBuff + count_; }
};
__host__ __device__ constexpr auto operator- (const RandomAccessIter &lhs, int* rhs) { return base(lhs) - rhs; }
__host__ __device__ constexpr auto operator- (int* lhs, const RandomAccessIter &rhs) { return lhs - base(rhs); }
__host__ __device__ constexpr bool operator==(const RandomAccessIter &lhs, int* rhs) { return base(lhs) == rhs; }
#if TEST_STD_VER < 2020
__host__ __device__ constexpr bool operator==(int* lhs, const RandomAccessIter &rhs) { return base(rhs) == lhs; }
__host__ __device__ constexpr bool operator!=(const RandomAccessIter &lhs, int* rhs) { return base(lhs) != rhs; }
__host__ __device__ constexpr bool operator!=(int* lhs, const RandomAccessIter &rhs) { return base(rhs) != lhs; }
#endif
...
struct TimesTwo {
  __host__ __device__ constexpr int operator()(int x) const { return x * 2; }
};

struct PlusOneMutable {
  __host__ __device__ constexpr int operator()(int x) { return x + 1; }
};

struct PlusOne {
  __host__ __device__ constexpr int operator()(int x) const { return x + 1; }
};

struct Increment {
  __host__ __device__ constexpr int& operator()(int& x) { return ++x; }
};

struct IncrementRvalueRef {
  __host__ __device__ constexpr int&& operator()(int& x) { return cuda::std::move(++x); }
};

struct PlusOneNoexcept {
  __host__ __device__ constexpr int operator()(int x) noexcept { return x + 1; }
};
