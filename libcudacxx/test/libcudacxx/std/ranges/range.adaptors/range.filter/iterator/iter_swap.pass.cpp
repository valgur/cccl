...
concept has_iter_swap = requires (It it) {
  cuda::std::ranges::iter_swap(it, it);
};
#else
...
template <class It>
inline constexpr bool has_iter_swap<It, cuda::std::void_t<decltype(cuda::std::ranges::iter_swap(cuda::std::declval<It>(), cuda::std::declval<It>()))>> = true;
#endif

struct IsEven {
  __host__ __device__ constexpr bool operator()(int x) const { return x % 2 == 0; }
};
