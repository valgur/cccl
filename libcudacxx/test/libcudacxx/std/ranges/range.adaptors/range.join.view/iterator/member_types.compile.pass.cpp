...
#else
  __host__ __device__ friend constexpr bool operator==(const diff_type_iter&, const diff_type_iter&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(const diff_type_iter&, const diff_type_iter&) { return false; }
#endif
};

template <class T, class V = int>
struct DiffTypeRange : cuda::std::ranges::view_base {
  __host__ __device__ diff_type_iter<T, V> begin() const { return diff_type_iter<T, V>{}; }
  __host__ __device__ diff_type_iter<T, V> end() const { return diff_type_iter<T, V>{}; }
};
...
__host__ __device__ void test() {
...

  {
    ...
    using Iter = cuda::std::ranges::iterator_t<cuda::std::ranges::join_view<Outer>>;
    static_assert(cuda::std::is_same_v<Iter::difference_type, cuda::std::common_type_t<cuda::std::intptr_t, cuda::std::ptrdiff_t>>);
  }
}
