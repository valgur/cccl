...
struct ConstComparableSentinel {
  ...
  __host__ __device__ constexpr explicit ConstComparableSentinel(const Iter& it) : iter_(it) {}
...
struct ConstComparableView : BufferView<BufferView<int*>*> {
  ...
  ConstComparableView(T&& arr)
    noexcept(noexcept(BufferView<BufferView<int*>*>(cuda::std::declval<T>())))
  ...
  __host__ __device__ constexpr auto begin() { return Iterator(data_); }
  __host__ __device__ constexpr auto begin() const { return ConstIterator(data_); }
  __host__ __device__ constexpr auto end() { return ConstComparableSentinel<false>(Iterator(data_ + size_)); }
  __host__ __device__ constexpr auto end() const { return ConstComparableSentinel<true>(ConstIterator(data_ + size_)); }
};
...
__host__ __device__ constexpr bool test() {
...

  {
    ...
    static_assert(!EqualityComparable<decltype(cuda::std::as_const(jv).begin()), decltype(jv.end())>);
    static_assert(!EqualityComparable<decltype(jv.begin()), decltype(cuda::std::as_const(jv).end())>);
  }

  {
    CopyableChild children[4] = {CopyableChild(buffer[0]), CopyableChild(buffer[1]), CopyableChild(buffer[2]),
                                 CopyableChild(buffer[3])};
    const auto jv = cuda::std::ranges::join_view(ParentView(children));
    assert(jv.end() == cuda::std::ranges::next(jv.begin(), 16));
  }
