...
struct convertible_sentinel_wrapper {
  ...
  __host__ __device__ constexpr convertible_sentinel_wrapper(const T& it) : it_(it) {}

  template <class U, cuda::std::enable_if_t<cuda::std::convertible_to<const U&, T>, int> = 0>
  __host__ __device__ constexpr convertible_sentinel_wrapper(const convertible_sentinel_wrapper<U>& other) : it_(other.it_) {}
...
struct ConstConveritbleView : BufferView<BufferView<int*>*> {
  ...
  __host__ __device__ constexpr BufferView<int*>* begin() { return data_; }
  __host__ __device__ constexpr const BufferView<int*>* begin() const { return data_; }
  __host__ __device__ constexpr sentinel end() { return sentinel(data_ + size_); }
  __host__ __device__ constexpr const_sentinel end() const { return const_sentinel(data_ + size_); }
};
...
LIBCPP_STATIC_ASSERT(!cuda::std::ranges::__simple_view<ConstConveritbleView>);
