...
struct View : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr explicit View(int start) : start_(start) {}
  __host__ __device__ constexpr View(View&& other) noexcept(IsNoexcept) : start_(cuda::std::exchange(other.start_, 0)) {}
...
struct CopyableView : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr CopyableView(CopyableView const& other) noexcept(IsNoexcept) : start_(other.start_) {}
  __host__ __device__ constexpr CopyableView& operator=(CopyableView const& other) noexcept(IsNoexcept) {
    start_ = other.start_;
    return *this;
  }
  __host__ __device__ constexpr explicit CopyableView(int start) noexcept : start_(start) {}
  __host__ __device__ constexpr int* begin() const { return globalBuff + start_; }
  __host__ __device__ constexpr int* end() const { return globalBuff + 8; }
};
...
struct Range {
  int start_;
  __host__ __device__ constexpr explicit Range(int start) noexcept : start_(start) {}
  __host__ __device__ constexpr int* begin() const { return globalBuff + start_; }
  __host__ __device__ constexpr int* end() const { return globalBuff + 8; }
};
...
struct BorrowableRange {
  int start_;
  __host__ __device__ constexpr explicit BorrowableRange(int start) noexcept : start_(start) {}
  __host__ __device__ constexpr int* begin() const { return globalBuff + start_; }
  __host__ __device__ constexpr int* end() const { return globalBuff + 8; }
};
template<>
...
struct RandomAccessRange {
  __host__ __device__ constexpr auto begin() { return random_access_iterator<int*>(globalBuff); }
  __host__ __device__ constexpr auto end() { return sized_sentinel(random_access_iterator<int*>(globalBuff + 8)); }
};
template<>
...
// old GCC seems to fall over the noexcept clauses here
 ...
 && (!defined(TEST_COMPILER_MSVC))
  ...
  {
    ASSERT_SAME_TYPE(decltype(cuda::std::views::all(cuda::std::declval<const CopyableView<true>&>())), CopyableView<true>);
...
int main(int, char**) {
  ...
  static_assert(test());
#endif // TEST_STD_VER > 2017 && _LIBCUDACXX_ADDRESSOF
