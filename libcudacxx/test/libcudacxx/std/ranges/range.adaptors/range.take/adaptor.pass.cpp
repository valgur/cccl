...
__host__ __device__ constexpr bool test() {
...
#ifndef TEST_COMPILER_NVCC // crashes nvcc
  ...
  // `views::take(subrange, n)` doesn't return a `subrange` if it's not a random access range.
  {
    ...
    using Result = cuda::std::ranges::take_view<cuda::std::ranges::subrange<forward_iterator<int*>,
  ...
  // Test that it's possible to call `cuda::std::views::take` with any single argument as long as the resulting closure is
