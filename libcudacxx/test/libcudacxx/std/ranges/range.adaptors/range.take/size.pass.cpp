...
template<class T>
concept SizeEnabled = requires(const cuda::std::ranges::take_view<T>& tv) {
  tv.size();
};
#else
...
template<class T>
inline constexpr bool SizeEnabled<T, cuda::std::void_t<decltype(cuda::std::declval<const cuda::std::ranges::take_view<T>&>().size())>> = true;
#endif
