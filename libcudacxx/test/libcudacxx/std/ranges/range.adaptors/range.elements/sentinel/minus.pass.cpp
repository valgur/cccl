...
struct SizedSent {
  ...
  __host__ __device__ friend constexpr bool operator==(const SizedSent& s, const Iter<Const>& i) { return i.it_ == s.end_; }
#if TEST_STD_VER < 2020
  __host__ __device__ friend constexpr bool operator==(const Iter<Const>& i, const SizedSent& s) { return i.it_ == s.end_; }
  __host__ __device__ friend constexpr bool operator!=(const SizedSent& s, const Iter<Const>& i) { return i.it_ != s.end_; }
  __host__ __device__ friend constexpr bool operator!=(const Iter<Const>& i, const SizedSent& s) { return i.it_ != s.end_; }
#endif

  __host__ __device__ friend constexpr intptr_t operator-(const SizedSent& st, const Iter<Const>& it) { return st.end_ - it.it_; }
  __host__ __device__ friend constexpr intptr_t operator-(const Iter<Const>& it, const SizedSent& st) { return it.it_ - st.end_; }
};
...
struct CrossSizedSent {
  ...
  template <bool C>
  __host__ __device__ friend constexpr bool operator==(const CrossSizedSent& s, const Iter<C>& i) { return i.it_ == s.end_; }
#if TEST_STD_VER < 2020
  template <bool C>
  __host__ __device__ friend constexpr bool operator==(const Iter<C>& i, const CrossSizedSent& s) { return i.it_ == s.end_; }
  template <bool C>
  __host__ __device__ friend constexpr bool operator!=(const CrossSizedSent& s, const Iter<C>& i) { return i.it_ != s.end_; }
  template <bool C>
  __host__ __device__ friend constexpr bool operator!=(const Iter<C>& i, const CrossSizedSent& s) { return i.it_ != s.end_; }
  ...
  template <bool C>
  __host__ __device__ friend constexpr intptr_t operator-(const CrossSizedSent& st, const Iter<C>& it) { return st.end_ - it.it_; }
  template <bool C>
  __host__ __device__ friend constexpr intptr_t operator-(const Iter<C>& it, const CrossSizedSent& st) { return it.it_ - st.end_; }
};
...
struct Range : TupleBufferView {
  ...
  __host__ __device__ constexpr sentinel end() { return sentinel{buffer_ + size_}; }
  __host__ __device__ constexpr const_sentinel end() const { return const_sentinel{buffer_ + size_}; }
};
...
template <class T, class U>
inline constexpr bool HasMinus<T, U, cuda::std::void_t<decltype(cuda::std::declval<const T>() - cuda::std::declval<const U>())>> = true;
#endif
