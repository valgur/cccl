...
template <class T>
concept IsBaseNoexcept = requires { { cuda::std::declval<T>().base() } noexcept; };
#else
template<class T, class = void>
inline constexpr bool IsBaseNoexcept = false;

template<class T>
inline constexpr bool IsBaseNoexcept<T, cuda::std::void_t<decltype(cuda::std::declval<T>().base())>> = noexcept(cuda::std::declval<T>().base());
#endif
...
struct Sent {
  __host__ __device__ friend constexpr bool operator==(const Sent&, const MoveOnlyIter&) { return true; }
#if TEST_STD_VER < 2020
  __host__ __device__ friend constexpr bool operator==(const MoveOnlyIter&, const Sent&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(const Sent&, const MoveOnlyIter&) { return false; }
  __host__ __device__ friend constexpr bool operator!=(const MoveOnlyIter&, const Sent&) { return false; }
#endif
};
