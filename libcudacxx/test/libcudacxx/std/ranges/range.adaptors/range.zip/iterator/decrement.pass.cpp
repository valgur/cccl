...
struct NonBidi : IntBufferView {
  ...
  __host__ __device__ constexpr NonBidi(int (&b)[N]) : IntBufferView(b) {}
#else
  using IntBufferView::IntBufferView;
#endif
  using iterator = forward_iterator<int*>;
  __host__ __device__ constexpr iterator begin() const { return iterator(buffer_); }
  __host__ __device__ constexpr sentinel_wrapper<iterator> end() const { return sentinel_wrapper<iterator>(iterator(buffer_ + size_)); }
};
