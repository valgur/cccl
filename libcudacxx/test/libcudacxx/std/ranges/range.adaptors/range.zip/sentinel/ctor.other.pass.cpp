...
struct NonSimpleNonCommonConvertibleView : IntBufferView {
  ...
  __host__ __device__ constexpr NonSimpleNonCommonConvertibleView(int (&b)[N]) : IntBufferView(b) {}
#else
  ...
  __host__ __device__ constexpr convertible_sentinel_wrapper<int*> end() { return convertible_sentinel_wrapper<int*>(buffer_ + size_); }
...
LIBCPP_STATIC_ASSERT(!cuda::std::ranges::__simple_view<NonSimpleNonCommonConvertibleView>);
