...
struct convertible_forward_sized_iterator {
  ...
  __host__ __device__ constexpr convertible_forward_sized_iterator(Base it) : it_(it) {}

  template <class U, cuda::std::enable_if_t<cuda::std::convertible_to<U, Base>, int> = 0>
  __host__ __device__ constexpr convertible_forward_sized_iterator(const convertible_forward_sized_iterator<U>& it) : it_(it.it_) {}

  __host__ __device__ constexpr decltype(*Base{}) operator*() const { return *it_; }

  __host__ __device__ constexpr convertible_forward_sized_iterator& operator++() {
    ++it_;
    return *this;
  }
  __host__ __device__ constexpr convertible_forward_sized_iterator operator++(int) { return forward_sized_iterator<Base>(it_++); }

#if defined(TEST_COMPILER_GCC) && __GNUC__ < 9 // Old gcc has issues establishing the conversion sequence
  __host__ __device__ constexpr convertible_forward_sized_iterator(forward_sized_iterator<Base> it) : it_(it.it_) {}
#endif // defined(TEST_COMPILER_GCC) && __GNUC__ < 9

#if TEST_STD_VER > 2017
  __host__ __device__ friend constexpr bool operator==(
      const convertible_forward_sized_iterator&, const convertible_forward_sized_iterator&) = default;
#else
  __host__ __device__ friend constexpr bool operator==(
      const convertible_forward_sized_iterator& lhs, const convertible_forward_sized_iterator& rhs) {
  ...
  __host__ __device__ friend constexpr difference_type operator-(const convertible_forward_sized_iterator& x,
...
struct convertible_sized_sentinel {
  ...
  __host__ __device__ constexpr convertible_sized_sentinel(const Base& it) : base_(it) {}

  template <class U, cuda::std::enable_if_t<cuda::std::convertible_to<U, Base>, int> = 0>
  __host__ __device__ constexpr convertible_sized_sentinel(const convertible_sized_sentinel<U>& other) : base_(other.base_) {}

  template <class U, cuda::std::enable_if_t<cuda::std::convertible_to<Base, U> || cuda::std::convertible_to<U, Base>, int> = 0>
...
struct ConstCompatibleForwardSized : IntBufferView {
  ...
  __host__ __device__ constexpr ConstCompatibleForwardSized(int (&b)[N]) : IntBufferView(b) {}
#else
  ...
  __host__ __device__ constexpr iterator begin() { return {buffer_}; }
  __host__ __device__ constexpr const_iterator begin() const { return {buffer_}; }
  __host__ __device__ constexpr convertible_sized_sentinel<iterator> end() { return iterator{buffer_ + size_}; }
  __host__ __device__ constexpr convertible_sized_sentinel<const_iterator> end() const { return const_iterator{buffer_ + size_}; }
};
