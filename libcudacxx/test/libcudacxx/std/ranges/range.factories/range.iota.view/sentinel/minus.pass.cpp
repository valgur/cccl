...
template<class T>
concept MinusInvocable = requires(cuda::std::ranges::iota_view<T, IntSentinelWith<T>> io) {
  io.end() - io.begin();
};
#else
...
template<class T>
inline constexpr bool MinusInvocable<T, cuda::std::void_t<
  decltype(cuda::std::declval<cuda::std::ranges::iota_view<T, IntSentinelWith<T>>>().end() -
           cuda::std::declval<cuda::std::ranges::iota_view<T, IntSentinelWith<T>>>().begin())>> = true;
#endif
