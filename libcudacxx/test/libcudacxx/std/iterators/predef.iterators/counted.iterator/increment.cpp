...
public:
    ...
    ThrowsOnInc() = default;
    __host__ __device__ explicit constexpr ThrowsOnInc(It it) : it_(it) {}

    __host__ __device__ constexpr reference operator*() const {return *it_;}

    __host__ __device__ constexpr ThrowsOnInc& operator++() {throw 42;}
    __host__ __device__ constexpr ThrowsOnInc operator++(int) {throw 42;}
};
...
template<class Iter>
inline constexpr bool PlusEnabled<Iter, cuda::std::void_t<decltype(cuda::std::declval<Iter&>()++),
