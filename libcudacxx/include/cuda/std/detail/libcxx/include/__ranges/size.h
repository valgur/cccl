...
_LIBCUDACXX_BEGIN_NAMESPACE_CPO(__size)
  ...
  _LIBCUDACXX_CONCEPT_FRAGMENT(
    ...
    requires(_Tp&& __t)(
      ...
      requires(sized_sentinel_for<decltype(_CUDA_VRANGES::end(__t)), decltype(_CUDA_VRANGES::begin(_CUDA_VSTD::declval<_Tp>()))>)
    ));
  ...
  struct __fn {
    ...
    constexpr size_t operator()(_Tp (&&)[_Sz]) const noexcept {
      return _Sz;
    }
...
#if _CCCL_STD_VER > 2017
  template<class _Tp>
  concept __can_ssize = requires (_Tp&& __t) {
    _CUDA_VRANGES::size(__t);
  };
#else // ^^^ CXX20 ^^^ / vvv CXX17 vvv
  template<class _Tp>
  _LIBCUDACXX_CONCEPT_FRAGMENT(
