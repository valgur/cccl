...
public:
  ...
  _LIBCUDACXX_TEMPLATE(class _I2, class _S2)
    _LIBCUDACXX_REQUIRES(convertible_to<const _I2&, _Iter> _LIBCUDACXX_AND convertible_to<const _S2&, _Sent> _LIBCUDACXX_AND
  ...
  _LIBCUDACXX_TEMPLATE(class _I2 = _Iter)
    _LIBCUDACXX_REQUIRES(indirectly_readable<const _I2> _LIBCUDACXX_AND (__has_const_arrow<_I2> || is_reference_v<iter_reference_t<_I2>> ||
  ...
  friend constexpr auto operator==(const common_iterator& __x, const common_iterator<_I2, _S2>& __y)
    _LIBCUDACXX_TRAILING_REQUIRES(bool)(sentinel_for<_S2, _Iter> && sentinel_for<_Sent, _I2> &&
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY
  friend constexpr auto operator-(const common_iterator& __x, const common_iterator<_I2, _S2>& __y)
    _LIBCUDACXX_TRAILING_REQUIRES(iter_difference_t<_I2>)(sized_sentinel_for<_I2, _Iter> && sized_sentinel_for<_S2, _Iter> &&
